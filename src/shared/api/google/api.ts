...
import {
  GooglePlaceDetailReq,
  GooglePlaceListReq,
  GooglePlaceSearchReq,
...
export class Api {
  getGooglePlaceList = ({
    location,
    category,
  }: GooglePlaceListReq): Promise<GooglePlaceListRes> =>
  ...
  getGooglePlaceDetail = ({
    place_id,
  }: GooglePlaceDetailReq): Promise<GooglePlaceDetailRes> =>
    axiosInstanceGoogleApi.get('/place/details/json', {
      params: {
        place_id: place_id,
        fields:
          'name,formatted_address,rating,user_ratings_total,url,place_id,types,geometry',
      },
    });
  searchGooglePlaces = ({
    query,
    type,
  }: GooglePlaceSearchReq): Promise<GooglePlaceSearchRes> =>
